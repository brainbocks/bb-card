...
export class CardInputComponent implements OnInit, ControlValueAccessor, Validator {
  ...
  public onPaste(event: ClipboardEvent): void {
    event.preventDefault();
    const caretPos = (event.target as HTMLInputElement).selectionStart;// get the current position of the caret in the input
    const data = event.clipboardData?.getData('text');
    if(!(data && caretPos)) return;
    let value = '';
  ...
  */
  ...
  /** InterPayment: 636 */
  private isInterPayment(cardNumber: string): boolean {
